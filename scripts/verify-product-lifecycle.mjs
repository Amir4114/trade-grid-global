...
async function provisionAdmin() {
  ...
  if (created.error)
    fatal(`admin provisioning failed: ${created.error.message}`)
  ...
  if (promote.error)
    fatal(`admin role assignment failed: ${promote.error.message}`)
  return adminEmail
}
...
try {
  ...
  if (submitPending.error)
    fatal(`submit pending failed: ${submitPending.error.message}`)
  ...
  if (adminEmail) {
    ...
    const publicAfter = await supabase
      ...
      .eq("id", publishCandidate.id)
      .maybeSingle()
    check("Reopened product is no longer publicly visible", !publicAfter.data)
  } else {
    skip(
