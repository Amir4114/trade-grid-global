...
try {
  ...
  if (probe.error) {
    ...
    if (
      /does not exist|schema cache|Could not find the table|PGRST205|relation/i.test(
        msg
      )
    ) {
      console.error(
  ...
  check(
    "public_products view is reachable (migration 008 applied)",
    !probe.error
  )

  const suppliersProbe = await supabase
    .from("public_suppliers")
    .select("company_id")
    .limit(1)
  check(
    "public_suppliers view is reachable",
    !suppliersProbe.error,
    suppliersProbe.error?.message
  )

  // ---- view column exposure (no private leakage) -------------------------
  const leakUserId = await supabase
    .from("public_products")
    .select("user_id")
    .limit(1)
  check(
    "public_products does NOT expose user_id",
    !!leakUserId.error,
    leakUserId.error?.message
  )
  const leakRisk = await supabase
    .from("public_products")
    .select("risk_score")
    .limit(1)
  check(
    "public_products does NOT expose risk_score",
    !!leakRisk.error,
    leakRisk.error?.message
  )
  const leakSupUser = await supabase
    .from("public_suppliers")
    .select("user_id")
    .limit(1)
  check(
    "public_suppliers does NOT expose user_id",
    !!leakSupUser.error,
    leakSupUser.error?.message
  )
  ...
  const anonUpload = await supabase.storage
    .from(BUCKET)
    .upload(`${supplier1.companyId}/anon-${stamp}.png`, PNG, {
      contentType: "image/png",
    })
  check(
    "Anonymous user CANNOT upload product images",
    !!anonUpload.error,
    anonUpload.error?.message
  )
  ...
  const buyerUpload = await supabase.storage
    .from(BUCKET)
    .upload(`${buyer.companyId}/buyer-${stamp}.png`, PNG, {
      contentType: "image/png",
    })
  check(
    "Buyer CANNOT upload product images",
    !!buyerUpload.error,
    buyerUpload.error?.message
  )
  ...
  const crossUpload = await supabase.storage
    .from(BUCKET)
    .upload(`${supplier2.companyId}/cross-${stamp}.png`, PNG, {
      contentType: "image/png",
    })
  check(
  ...
  skip(
    "Published product appears in public_products with supplier identity - requires admin publish (service-role/manual)"
  )
