...
async function assertMigrationsApplied() {
  ...
  if (
    /Could not find the table|relation .* does not exist|42P01/i.test(poMsg)
  ) {
    console.error(
...
try {
  ...
  console.log(
    "PASS - Migration 017 purchase_orders / create_purchase_order_draft reachable"
  )
  passed++
  ...
  const eventsProbe = await supabase
    ...
    .limit(1)
  check(
    "purchase_order_events readable under RLS",
    !eventsProbe.error,
    eventsProbe.error
  )

  const itemsProbe = await supabase
    .from("purchase_order_items")
    .select("id")
    .limit(1)
  check(
    "purchase_order_items readable under RLS",
    !itemsProbe.error,
    itemsProbe.error
  )
  ...
  check(
    "One active PO per award (duplicate draft denied)",
    !!dup.error,
    dup.error?.message
  )
  ...
  const peerSelect = await supabase
    .from("purchase_orders")
    .select("id")
    .eq("id", poId)
  check(
  ...
  // Notifications
  const createdNotif = await countNotifications(
    buyer.userId,
    "purchase_order.created",
    poId
  )
  if (createdNotif == null)
    skip("purchase_order.created notification", "no service role")
  else
    check("Buyer receives purchase_order.created", createdNotif >= 1, {
      createdNotif,
    })

  const issuedNotif = await countNotifications(
    winner.userId,
    "purchase_order.issued",
    poId
  )
  if (issuedNotif == null)
    skip("purchase_order.issued notification", "no service role")
  else
    check("Supplier receives purchase_order.issued", issuedNotif >= 1, {
      issuedNotif,
    })

  const acceptedNotif = await countNotifications(
    buyer.userId,
    "purchase_order.accepted",
    poId
  )
  if (acceptedNotif == null)
    skip("purchase_order.accepted notification", "no service role")
  else
    check("Buyer receives purchase_order.accepted", acceptedNotif >= 1, {
      acceptedNotif,
    })
  ...
  await signIn(buyer.email)
  const rfq4 = await createAndPublishRfq({
    title: `PO draft revoke RFQ ${stamp}`,
  })
  await signIn(winner.email)
