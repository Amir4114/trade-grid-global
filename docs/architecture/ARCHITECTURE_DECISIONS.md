...
| ----------- | ----------------------------------------------------------- | ------ |
| AD-TV-001   | Verification cases own the review lifecycle                 | LOCKED |
| AD-TV-002   | Every case freezes its submitted evidence set               | LOCKED |
| AD-TV-003   | Human review precedes a reasoned terminal decision          | LOCKED |
| AD-TV-004   | Baseline and conditional evidence remain distinct           | LOCKED |
| AD-TV-005   | Material changes invalidate trust; risk remains independent | LOCKED |
| AD-TV-006   | Onboarding owns company evidence collection                 | LOCKED |

---

## Index — Marketplace Experience

| Decision ID | Title                                                    | Status |
| ----------- | -------------------------------------------------------- | ------ |
| AD-ME-001   | Guest access is a temporary non-account browsing context | LOCKED |
| AD-ME-002   | Workspace UX consumes but never owns verification state  | LOCKED |
| AD-ME-003   | Public company pages use the safe marketplace projection | LOCKED |
| AD-ME-004   | Workspace summaries are derived presentation projections | LOCKED |
...
| AD-3.2-029  | Milestones and comments are append-only fulfillment events   | LOCKED |
...
### AD-3.2-029 — Milestones and comments are append-only fulfillment events

| Field                       | Value                                                                                                                                                                                                                                                                       |
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Decision ID**             | AD-3.2-029                                                                                                                                                                                                                                                                  |
| **Module**                  | 3.2 Order Lifecycle — Phase B                                                                                                                                                                                                                                               |
| **Title**                   | Milestones and comments are append-only fulfillment events                                                                                                                                                                                                                  |
| **Status**                  | LOCKED                                                                                                                                                                                                                                                                      |
| **Decision**                | Operational milestones and party comments are typed rows in `fulfillment_order_events`. Milestones store their canonical type and occurrence time in metadata; comments store author role. Neither changes status unless an existing lifecycle RPC performs the transition. |
| **Reason**                  | Both are immutable timeline facts without independent lifecycle, ownership, or relationships. Reusing the event contract preserves chronology, actor attribution, notification integration, and future event consumers without duplicating operational truth.               |
| **Alternatives considered** | A mutable milestones array on `fulfillment_orders` (rejected — overwrites history); a separate milestone table (rejected — unnecessary aggregate and duplicated actor/timeline policy); logistics statuses for ports/customs (rejected — Module 3.3 ownership).             |
| **Future review**           | Attachments may reference event IDs after a governed uploader-retention contract exists. External event publication requires idempotency and delivery semantics.                                                                                                            |
| **Date**                    | 2026-07-19                                                                                                                                                                                                                                                                  |
...
### AD-ID-001 — Marketplace signup is provisioned inside the Auth transaction

| Field                       | Value                                                                                                                                                                                                                                                                                                                                            |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Decision ID**             | AD-ID-001                                                                                                                                                                                                                                                                                                                                        |
| **Module**                  | Identity and Access                                                                                                                                                                                                                                                                                                                              |
| **Title**                   | Marketplace signup is provisioned inside the Auth transaction                                                                                                                                                                                                                                                                                    |
| **Status**                  | LOCKED                                                                                                                                                                                                                                                                                                                                           |
| **Decision**                | A marked marketplace signup validates Buyer or Supplier metadata and creates `profiles` plus `companies` from an `auth.users` trigger. Auth and both public records commit or roll back together. Client-side sequential provisioning is forbidden. Legacy incomplete users may use one authenticated recovery RPC only while no company exists. |
| **Reason**                  | Authentication without its role and company creates an unusable, incorrectly routed identity. A database trigger shares the Auth insert transaction; client requests and compensating deletion do not.                                                                                                                                           |
| **Alternatives considered** | Client Auth followed by profile/company UPSERTs (rejected — non-atomic); service-role cleanup after failure (rejected — compensation can fail); business fields on `auth.users` (rejected — couples marketplace data to the identity provider).                                                                                                  |
| **Future review**           | Multi-company memberships require a separate membership domain and must not weaken this initial-account invariant.                                                                                                                                                                                                                               |
| **Date**                    | 2026-07-18                                                                                                                                                                                                                                                                                                                                       |

---

## Decisions — Trust & Verification

### AD-TV-001 — Verification cases own the review lifecycle

| Field                       | Value                                                                                                                                                                                                                                                         |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Decision ID**             | AD-TV-001                                                                                                                                                                                                                                                     |
| **Module**                  | Trust & Verification                                                                                                                                                                                                                                          |
| **Title**                   | Verification cases own the review lifecycle                                                                                                                                                                                                                   |
| **Status**                  | LOCKED                                                                                                                                                                                                                                                        |
| **Decision**                | `companies` owns current eligibility projection (`pending`, `under_review`, `verified`, `rejected`). Each review attempt is a distinct `verification_cases` record with immutable events. Terminal cases are never reopened; resubmission creates a new case. |
| **Reason**                  | Company identity is durable, while verification is repeatable. Keeping attempts separate preserves reviewer accountability, rejection history, renewal readiness, and audit reconstruction.                                                                   |
| **Alternatives considered** | Add reviewer/reason/history columns to `companies` (rejected — overwrites prior attempts); reuse one case forever (rejected — destroys attempt boundaries).                                                                                                   |
| **Future review**           | Annual renewal may add a case purpose/cycle without changing ownership.                                                                                                                                                                                       |
| **Date**                    | 2026-07-18                                                                                                                                                                                                                                                    |

### AD-TV-002 — Every case freezes its submitted evidence set

| Field                       | Value                                                                                                                                                                                                                                                                                                                                                                                 |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Decision ID**             | AD-TV-002                                                                                                                                                                                                                                                                                                                                                                             |
| **Module**                  | Trust & Verification                                                                                                                                                                                                                                                                                                                                                                  |
| **Title**                   | Every case freezes its submitted evidence set                                                                                                                                                                                                                                                                                                                                         |
| **Status**                  | LOCKED                                                                                                                                                                                                                                                                                                                                                                                |
| **Decision**                | Submission records an immutable association between the active verification case and every active document reviewed for that attempt. Admin case views and decisions use that case-scoped set, not a live query of all company documents. Owners may replace/delete only pending evidence that has never been case-linked; rejected, approved, and case-linked evidence is immutable. |
| **Reason**                  | Evidence uploaded after submission must not silently alter an in-progress or historical decision. Case-scoped evidence also makes rejection/replacement and future renewal auditable.                                                                                                                                                                                                 |
| **Alternatives considered** | Query all current company documents (rejected — mutable review scope); store a single `case_id` on each document (rejected — prevents safe reuse of approved evidence across later attempts).                                                                                                                                                                                         |
| **Future review**           | Add content hashes, issuer, jurisdiction, validity dates, malware scan results, and legal-retention controls.                                                                                                                                                                                                                                                                         |
| **Date**                    | 2026-07-18                                                                                                                                                                                                                                                                                                                                                                            |

### AD-TV-003 — Human review precedes a reasoned terminal decision

| Field                       | Value                                                                                                                                                                                                                                                                                                                                                   |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Decision ID**             | AD-TV-003                                                                                                                                                                                                                                                                                                                                               |
| **Module**                  | Trust & Verification                                                                                                                                                                                                                                                                                                                                    |
| **Title**                   | Human review precedes a reasoned terminal decision                                                                                                                                                                                                                                                                                                      |
| **Status**                  | LOCKED                                                                                                                                                                                                                                                                                                                                                  |
| **Decision**                | A company case follows `pending → in_review → approved or rejected`. Approval or rejection from `pending` is forbidden. Rejection requires a non-blank reason. Direct owner or admin verification-status writes cannot bypass trusted RPCs. Terminal decisions are not rolled back; corrections use a new case or a future audited break-glass process. |
| **Reason**                  | Review start captures assignment and review timing. Mandatory rejection feedback is required for a usable resubmission loop and defensible compliance operations.                                                                                                                                                                                       |
| **Alternatives considered** | Permit one-click decision from queue (rejected — incomplete review accountability); optional rejection reason (rejected — unactionable and weak audit).                                                                                                                                                                                                 |
| **Future review**           | Dual control for high-risk categories and audited break-glass correction.                                                                                                                                                                                                                                                                               |
| **Date**                    | 2026-07-18                                                                                                                                                                                                                                                                                                                                              |

### AD-TV-004 — Baseline and conditional evidence remain distinct

| Field                       | Value                                                                                                                                                                                                                                                                                                                                                     |
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Decision ID**             | AD-TV-004                                                                                                                                                                                                                                                                                                                                                 |
| **Module**                  | Trust & Verification                                                                                                                                                                                                                                                                                                                                      |
| **Title**                   | Baseline and conditional evidence remain distinct                                                                                                                                                                                                                                                                                                         |
| **Status**                  | LOCKED                                                                                                                                                                                                                                                                                                                                                    |
| **Decision**                | v0.4.1 universally requires `Trade License` and `Company Registration`. Tax/VAT, import/export licensing, food-safety certificates, ISO, Halal, Organic, and other certificates are optional evidence until a country/category policy matrix is approved. Product, shipment, and transaction evidence must not be relabeled as company identity evidence. |
| **Reason**                  | Two baseline documents match the implemented submission contract. Jurisdiction- and category-specific requirements cannot be safely treated as universal without policy ownership and applicability rules.                                                                                                                                                |
| **Alternatives considered** | Require every certificate globally (rejected — legally incorrect across jurisdictions); accept arbitrary free-text mandatory types (rejected — inconsistent enforcement).                                                                                                                                                                                 |
| **Future review**           | Governed evidence taxonomy and conditional requirements by country, company activity, product category, and trade direction.                                                                                                                                                                                                                              |
| **Date**                    | 2026-07-18                                                                                                                                                                                                                                                                                                                                                |

### AD-TV-005 — Material changes invalidate trust; risk remains independent

| Field                       | Value                                                                                                                                                                                                                                                                                                                                                                                                                |
| --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Decision ID**             | AD-TV-005                                                                                                                                                                                                                                                                                                                                                                                                            |
| **Module**                  | Trust & Verification                                                                                                                                                                                                                                                                                                                                                                                                 |
| **Title**                   | Material changes invalidate trust; risk remains independent                                                                                                                                                                                                                                                                                                                                                          |
| **Status**                  | LOCKED                                                                                                                                                                                                                                                                                                                                                                                                               |
| **Decision**                | Owner-created company rows always begin `pending` with unassessed risk `50`, and account type must match profile role. Changes to legal company name, registration country, business type, or company structure reset `verified or under_review → pending` and cancel any active case. Risk score uses `0` as lowest risk and `100` as highest risk. Verification approval does not automatically change risk score. |
| **Reason**                  | Trust cannot survive material identity changes without reassessment. Verification is an evidence decision, while risk is a separate governed assessment and must not be silently set to “lowest risk” on approval.                                                                                                                                                                                                   |
| **Alternatives considered** | Invalidate only name/country (rejected — misses legal/activity changes); set risk to zero on approval (rejected — conflates identity evidence with credit/fraud risk).                                                                                                                                                                                                                                               |
| **Future review**           | Renewal/expiry policy, risk model ownership, explainable scoring, AML/KYC, and third-party reassessment signals.                                                                                                                                                                                                                                                                                                     |
| **Date**                    | 2026-07-18                                                                                                                                                                                                                                                                                                                                                                                                           |

### AD-TV-006 — Onboarding owns company evidence collection

| Field                       | Value                                                                                                                                                                                                                                                                                                                                               |
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Decision ID**             | AD-TV-006                                                                                                                                                                                                                                                                                                                                           |
| **Module**                  | Trust & Verification                                                                                                                                                                                                                                                                                                                                |
| **Title**                   | Onboarding owns company evidence collection                                                                                                                                                                                                                                                                                                         |
| **Status**                  | LOCKED                                                                                                                                                                                                                                                                                                                                              |
| **Decision**                | Buyer and Supplier onboarding use one role-configured workspace for legal profile, categories, markets, certifications, company documents, review, and verification submission. `documents` remains the mutable pre-submission source; `verification_case_documents` remains the immutable submitted snapshot. Standalone upload UIs redirect here. |
| **Reason**                  | One collection workflow prevents divergent validation, unsupported document labels, and contradictory submission states while preserving role-specific trade profile fields.                                                                                                                                                                        |
| **Alternatives considered** | Separate Buyer/Supplier upload pages (rejected — duplicate policy and UX); store uploads directly on verification cases (rejected — a case must freeze a complete submission, not become a mutable draft container); extend certification arrays with files (rejected — profile claims are not evidence records).                                   |
| **Future review**           | Conditional evidence requirements may extend the shared configuration after a governed country/category policy matrix exists.                                                                                                                                                                                                                       |
| **Date**                    | 2026-07-19                                                                                                                                                                                                                                                                                                                                          |

### AD-ME-001 — Guest access is a temporary non-account browsing context

| Field                       | Value                                                                                                                                                                                                                                                                                                                   |
| --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Decision ID**             | AD-ME-001                                                                                                                                                                                                                                                                                                               |
| **Module**                  | Marketplace Experience                                                                                                                                                                                                                                                                                                  |
| **Title**                   | Guest access is a temporary non-account browsing context                                                                                                                                                                                                                                                                |
| **Status**                  | LOCKED                                                                                                                                                                                                                                                                                                                  |
| **Decision**                | Guest access uses a short-lived, authenticated-encrypted, HttpOnly cookie containing minimal name/email/session metadata. It creates no Supabase Auth user, profile, company, role, or Trust record. Guests can read public marketplace surfaces only; dashboard and trading writes remain authenticated/RLS protected. |
| **Reason**                  | A guest is an acquisition context, not a marketplace legal actor. Separation avoids orphan accounts, false company identities, and weakened tenant assumptions while providing guided read-only entry.                                                                                                                  |
| **Alternatives considered** | Create incomplete Auth users (rejected — repeats partial-provisioning risk); insert guest profiles/companies (rejected — pollutes Identity and Trust); unsigned client storage (rejected — permits session-content tampering).                                                                                          |
| **Future review**           | Consent/audit requirements may justify a dedicated lead-management integration separate from marketplace account provisioning.                                                                                                                                                                                          |
| **Date**                    | 2026-07-19                                                                                                                                                                                                                                                                                                              |

### AD-ME-002 — Workspace UX consumes but never owns verification state

| Field                       | Value                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Decision ID**             | AD-ME-002                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| **Module**                  | Marketplace Experience                                                                                                                                                                                                                                                                                                                                                                                                                           |
| **Title**                   | Workspace UX consumes but never owns verification state                                                                                                                                                                                                                                                                                                                                                                                          |
| **Status**                  | LOCKED                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| **Decision**                | Workspace Overview reads the canonical company verification projection. `under_review` users retain workspace, public marketplace access, and private draft authoring. Workspace UI gates public or commercially binding transitions such as publish, submit, award, and issue until `verified`. Navigation never writes verification status or duplicates the Trust lifecycle. Existing commercial RPC/RLS policy remains unchanged in Phase 1. |
| **Reason**                  | Review can take time and should not strand legitimate users, but unverified companies must not receive verified trading privileges. The Trust domain remains the sole transition authority.                                                                                                                                                                                                                                                      |
| **Alternatives considered** | Block the entire workspace during review (rejected — poor activation and no read-only security benefit); duplicate a workspace verification state (rejected — drift); bypass Trust checks after onboarding completion (rejected — weakens controls).                                                                                                                                                                                             |
| **Future review**           | A governed capability matrix may replace the initial verified/not-verified action gate as each commercial action receives an explicit risk classification.                                                                                                                                                                                                                                                                                       |
| **Date**                    | 2026-07-19                                                                                                                                                                                                                                                                                                                                                                                                                                       |

### AD-ME-003 — Public company pages use the safe marketplace projection

| Field                       | Value                                                                                                                                                                                                                                                                                                          |
| --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Decision ID**             | AD-ME-003                                                                                                                                                                                                                                                                                                      |
| **Module**                  | Marketplace Experience                                                                                                                                                                                                                                                                                         |
| **Title**                   | Public company pages use the safe marketplace projection                                                                                                                                                                                                                                                       |
| **Status**                  | LOCKED                                                                                                                                                                                                                                                                                                         |
| **Decision**                | `/company/{slug}` reads only `public_suppliers` and `public_products`. The canonical slug combines a normalized company name with the immutable company UUID. Public pages never query owner-only `companies`, create a duplicate profile table, or infer confidential markets, transactions, or trust claims. |
| **Reason**                  | The public views are the existing reviewed disclosure boundary. A friendly label supports discovery while the UUID gives collision-free identity and survives company renames through canonical redirect.                                                                                                      |
| **Alternatives considered** | Anonymous access to `companies` (rejected — broadens RLS exposure); a synchronized public-company table (rejected — duplicates identity and verification truth); name-only slugs (rejected — collisions and unstable identity).                                                                                |
| **Future review**           | Additional public fields require an explicit, privacy-reviewed extension of the public projection; the page must not bypass that contract.                                                                                                                                                                     |
| **Date**                    | 2026-07-19                                                                                                                                                                                                                                                                                                     |

### AD-ME-004 — Workspace summaries are derived presentation projections

| Field                       | Value                                                                                                                                                                                                                                                                  |
| --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Decision ID**             | AD-ME-004                                                                                                                                                                                                                                                              |
| **Module**                  | Marketplace Experience                                                                                                                                                                                                                                                 |
| **Title**                   | Workspace summaries are derived presentation projections                                                                                                                                                                                                               |
| **Status**                  | LOCKED                                                                                                                                                                                                                                                                 |
| **Decision**                | The shared workspace header derives role-specific counts from existing RLS-filtered records and canonical Profile/Company state. Greeting, summary counts, company health, recent activity, and pending tasks are never stored as Company, Trust, or Analytics fields. |
| **Reason**                  | These values are contextual UI projections. Deriving them prevents stale counters, duplicated lifecycle state, and accidental coupling between workspace presentation and domain ownership.                                                                            |
| **Alternatives considered** | Persist summary counters on `companies` (rejected — stale cross-domain data); create an Analytics schema during UI polish (rejected — out of scope and without governed metric lineage); duplicate role headers (rejected — inconsistent behavior).                    |
| **Future review**           | High-volume deployments may replace direct counts with governed read models or analytics projections while preserving the same presentation contract.                                                                                                                  |
| **Date**                    | 2026-07-19                                                                                                                                                                                                                                                             |
