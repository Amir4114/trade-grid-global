...
const FILTERS: Array<{ label: string; value: PurchaseOrderStatus | "all" }> = [
  { label: "All", value: "all" },
  { label: "Pending response", value: "issued" },
...
export default function SupplierPurchaseOrderList() {
  ...
  return (
    <DashboardShell
      role="supplier"
      title="Orders"
      description="Incoming purchase orders from buyers. Review issued POs and accept or reject with a reason."
      actions={
        <Button asChild variant="outline">
          <Link href="/dashboard/supplier/awards">Awards</Link>
        </Button>
      }
    >
              ...
              >
                <div className="min-w-0">
                  ...
                  <div className="mt-1 text-sm text-neutral-700">
                    {row.product_name} · {row.buyer_company_name}
                  </div>
                ...
                <Button asChild size="sm" variant="outline">
                  <Link href={`/dashboard/supplier/orders/${row.id}`}>
                    {row.status === "issued" ? "Respond" : "Open"}
                  </Link>
                </Button>
              </li>
            ))}
