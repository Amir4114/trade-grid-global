...
export default function SupplierQuotationList() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        setLoading(true);
        const data = await listSupplierQuotationThreads(supabase);
        if (!active) return;
        setRows(data);
        setError(null);
      } catch (err) {
  ...
  return (
    <DashboardShell
      role="supplier"
      title="My Quotations"
      description="Track drafts, submitted offers, and revision history for each RFQ."
      actions={
        <Button asChild variant="outline">
          <Link href="/dashboard/supplier/rfqs">Discover RFQs</Link>
        </Button>
      }
    >
      <DashboardPanel title="Quotation threads">
        {error ? (
          ...
          <div className="py-16 text-center">
            ...
            <p className="mt-1 text-sm text-neutral-500">
              Open an eligible RFQ and submit a commercial offer.
            </p>
          </div>
        ) : (
                    ...
                    <tr key={row.id} className="hover:bg-neutral-50">
                      ...
                      <td className="px-4 py-3 text-right">
                        <Button asChild size="sm" variant="outline">
                          <Link href={`/dashboard/supplier/quotations/${row.id}`}>
                            Open
                          </Link>
                        </Button>
                      </td>
